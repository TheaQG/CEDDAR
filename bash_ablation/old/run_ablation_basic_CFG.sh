#!/bin/bash
#SBATCH --job-name=ab_basic_CFG2
#SBATCH --output=logs/slurm_ab_basic_CFG2_%x_%j.log
#SBATCH --error=logs/slurm_ab_basic_CFG2_%x_%j.err
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -m sbgm.cli.main_app --mode full_pipeline --config_path $CONFIG_DIR/ablation_basic_CFG.yaml --make_plots
  
"
  # python -m sbgm.cli.main_app --mode evaluate --config_path $CONFIG_DIR/ablation_basic_CFG.yaml --make_plots 
  # python -m sbgm.cli.main_app --mode generate --config_path $CONFIG_DIR/ablation_basic_CFG.yaml --make_plots &&
  # python -m sbgm.cli.main_app --mode sigma_star_generation --config_path $CONFIG_DIR/ablation_basic_CFG.yaml --make_plots &&
  # python -m sbgm.cli.main_app --mode sigma_star_evaluation --config_path $CONFIG_DIR/ablation_basic_CFG.yaml --make_plots
  # python -m sbgm.cli.main_app --mode quicklook --config_path '$CONFIG_DIR/ablation_basic_CFG.yaml' --make_plots &&
