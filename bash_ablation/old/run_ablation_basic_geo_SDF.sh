#!/bin/bash
#SBATCH --job-name=ab_basic_geo_SDF
#SBATCH --output=logs/slurm_ab_basic_geo_SDF_%x_%j.log
#SBATCH --error=logs/slurm_ab_basic_geo_SDF_%x_%j.err
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -m sbgm.cli.main_app --mode evaluate --config_path $CONFIG_DIR/ablation_basic_geo_SDF.yaml --make_plots
"
