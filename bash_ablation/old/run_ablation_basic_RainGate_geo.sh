#!/bin/bash
#SBATCH --job-name=ab_basic_RainGate_geo
#SBATCH --output=logs/slurm_ab_basic_RainGate_geo_%x_%j.log
#SBATCH --error=logs/slurm_ab_basic_RainGate_geo_%x_%j.err
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -m sbgm.cli.main_app --mode evaluate --config_path $CONFIG_DIR/ablation_basic_RainGate_geo.yaml --make_plots
"
  # python -m sbgm.cli.main_app --mode full_pipeline --config_path $CONFIG_DIR/ablation_basic_RainGate_geo.yaml --make_plots
  # python -m sbgm.cli.main_app --mode generate --config_path $CONFIG_DIR/ablation_basic_RainGate_geo.yaml --make_plots &&
  # python -m sbgm.cli.main_app --mode quicklook --config_path '$CONFIG_DIR/ablation_basic_RainGate_geo.yaml' --make_plots &&
