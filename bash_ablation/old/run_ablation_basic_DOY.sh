#!/bin/bash
#SBATCH --job-name=ab_basic_DOY
#SBATCH --output=logs/slurm_ab_basic_DOY_%x_%j.log
#SBATCH --error=logs/slurm_ab_basic_DOY_%x_%j.err
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -m sbgm.cli.main_app --mode evaluate --config_path $CONFIG_DIR/ablation_basic_DOY.yaml --make_plots
"
  ...
  # python -m sbgm.cli.main_app --mode full_pipeline --config_path $CONFIG_DIR/ablation_basic_DOY.yaml --make_plots
