#!/bin/bash
#SBATCH --job-name=ab_EDMResidualxGeo
#SBATCH --output=logs/slurm_ab_EDMResidualxGeo_%x_%j.log
#SBATCH --error=logs/slurm_ab_EDMResidualxGeo_%x_%j.err
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -m sbgm.cli.main_app --mode full_pipeline --config_path $CONFIG_DIR/ablation_EDMResidualxGeo.yaml --make_plots
"
