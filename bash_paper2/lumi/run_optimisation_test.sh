#!/bin/bash
#SBATCH --job-name=P0_optimisation_test
#SBATCH --output=logs/slurm_P0_optimisation_test_%x_%j.log
#SBATCH --error=logs/slurm_P0_optimisation_test_%x_%j.err
#SBATCH --account=project_465002493
#SBATCH --partition=standard-g
#SBATCH --nodes=1
#SBATCH --gpus-per-node=1
#SBATCH --cpus-per-task=7 # 56 # 7 * 8 cores per GPU
#SBATCH --mem-per-gpu=60G
#SBATCH --time=08:00:00


# ===================================================================
# P0_optimisation_test: B1_GSDF_RGBCE (BASELINE B1 + GEOGRAPHIC CONDITIONS AND SDF-WEIGHTED LOSS + RAINGATE BCE (WITHOUT REWEIGHTING))
# ===================================================================
# Final selected best model to run full eval pipeline on test dataset
...
CFG="$CONFIG_DIR/P0_optimise.yaml"

echo "[INFO] Running P0_optimisation_test: training → generation → quicklook → evaluation (inside container)"
...
# ------------------------------------------------------------------
export DEBUG_SINGLE_GPU="${DEBUG_SINGLE_GPU:-1}" 
export DDP_MULTI_GPU="${DDP_MULTI_GPU:-0}"
