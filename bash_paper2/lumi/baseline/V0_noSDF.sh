#!/bin/bash
#SBATCH --job-name=V0_noSDF
#SBATCH --output=logs/slurm_V0_noSDF_%x_%j.log
#SBATCH --error=logs/slurm_V0_noSDF_%x_%j.err
...
# ===================================================================
# V0_noSDF: B1_GSDF_RGBCE (BASELINE B1 + GEOGRAPHIC CONDITIONS AND SDF-WEIGHTED LOSS + RAINGATE BCE (WITHOUT REWEIGHTING), NO SDF-WEIGHTING IN LOSS)
# ===================================================================
# Baseline, no SDF-weighting in loss, paper 2
...
CFG="$CONFIG_DIR/baseline/V0_noSDF.yaml"

echo "[INFO] Running V0_noSDF: training → generation → quicklook → evaluation (inside container)"
