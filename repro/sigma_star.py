...
def main():
    ...
    parser.add_argument("action", choices=["prepare", "generate", "evaluate"])
    ...
    if args.action != "prepare":
        prepare_flags = {'--config', '--sigma-star-grid', '--sigma-star-mode', '--initial-state',
                         '--steps', '--ensemble-size', '--max-dates', '--split'}
        if any(token.split('=')[0] in prepare_flags for token in sys.argv[1:]):
            parser.error("Overrides apply only to prepare; generate/evaluate use resolved_config.yaml")
