#!/bin/bash
#SBATCH --job-name=F
#SBATCH --output=logs/slurm_F_%x_%j.log
#SBATCH --error=logs/slurm_F_%x_%j.err
...
# ===================================================================
# F: B1_GSDF_RGBCE (BASELINE B1 + GEOGRAPHIC CONDITIONS AND SDF-WEIGHTED LOSS + RAINGATE BCE (WITHOUT REWEIGHTING))
# ===================================================================
...
CONFIG_DIR="$ROOT_DIR/sbgm/config/component_study"
...
CFG="$CONFIG_DIR/F_final_test_eval.yaml"

echo "[INFO] Running ablation B1 + geographic conditions and SDF-weighted loss + RainGate BCE without reweighting: training → generation → quicklook → evaluation (inside container)"
