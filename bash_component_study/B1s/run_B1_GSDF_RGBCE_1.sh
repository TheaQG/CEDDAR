#!/bin/bash
#SBATCH --job-name=B1_GSDF_RGBCE_1
#SBATCH --output=logs/slurm_B1_GSDF_RGBCE_1_%x_%j.log
#SBATCH --error=logs/slurm_B1_GSDF_RGBCE_1_%x_%j.err
...
# ===================================================================
# B1_GSDF_RGBCE_1: BASELINE B1 + GEOGRAPHIC CONDITIONS AND SDF-WEIGHTED LOSS + RAINGATE BCE (WITHOUT REWEIGHTING) - REPLICATE WITH SEED 1011
# ===================================================================
...
CONFIG_DIR="$ROOT_DIR/sbgm/config/component_study/B1s"
...
CFG="$CONFIG_DIR/B1_GSDF_RGBCE_1.yaml"

echo "[INFO] REPLICATE: Running ablation B1 + geographic conditions and SDF-weighted loss + RainGate BCE without reweighting: training → generation → quicklook → evaluation (inside container)"
