#!/bin/bash
#SBATCH --job-name=retrains_noResidual
#SBATCH --output=logs/slurm_retrains_noResidual_%x_%j.log
#SBATCH --error=logs/slurm_retrains_noResidual_%x_%j.err
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -m sbgm.cli.main_app --mode full_pipeline --config_path $CONFIG_DIR/retrains_noResidual.yaml --make_plots &&
  python -m sbgm.cli.main_app --mode sigma_star_evaluation --config_path $CONFIG_DIR/retrains_noResidual.yaml --make_plots &&
  python -m sbgm.cli.main_app --mode sigma_star_generation --config_path $CONFIG_DIR/retrains_noResidual.yaml --make_plots
"

  # python -m sbgm.cli.main_app --mode generate --config_path $CONFIG_DIR/retrains_noResidual.yaml --make_plots &&
  # python -m sbgm.cli.main_app --mode evaluate --config_path $CONFIG_DIR/retrains_noResidual.yaml --make_plots &&
  # python -m sbgm.cli.main_app --mode quicklook --config_path '$CONFIG_DIR/retrains_noResidual.yaml' --make_plots &&
