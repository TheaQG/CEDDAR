...
def sample_high_impact(trial: optuna.trial.Trial) -> dict:
    ...
    """
    BLOCK_OPTIONS = [(2,2,2,2), (3,3,3,3), (3,4,6,3)]
    return {
        ...
        "sampler.block_layers": trial.suggest_categorical("block_layers", BLOCK_OPTIONS),
        # Only divisors of common fmap sizes (32, 64, 128,...)
        "sampler.num_heads":            trial.suggest_categorical("n_heads", [1, 2, 4, 8, 16]),
    }

def sample_medium_impact(trial: optuna.trial.Trial, cfg: dict) -> dict:
    """
    ...
    return {
        ...
        "training.weight_decay":        trial.suggest_float("wd", 1e-8, 1e-3, log=True),
        "sampler.last_fmap_channels":   trial.suggest_categorical("last_fmap_channels", [128, 256, 512]),
    }

from sqlalchemy import event

def create_sqlite_storage(path: str, timeout: int = 600) -> optuna.storages.BaseStorage:
    """
    Create an Optuna SQLite storage with WAL mode enabled.
    Helps the workers not overwrite
    """
    url = f"sqlite:///{path}"
    engine_kwargs = {"connect_args": {"timeout": timeout}}
    storage = optuna.storages.RDBStorage(url, engine_kwargs=engine_kwargs)

    # Turn on WAL so concurrent readers/writers don’t block each other
...
def objective(trial: optuna.trial.Trial,
    ...
    if enable_medium:
        deep_update(cfg, sample_medium_impact(trial, cfg))
...
if __name__ == "__main__":
    ...
    storage = create_sqlite_storage(pathlib.Path(args.storage[9:]))  # strip 'sqlite:///'
