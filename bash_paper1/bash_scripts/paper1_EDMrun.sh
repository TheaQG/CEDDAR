#!/bin/bash
#SBATCH --job-name=paper1_EDM_expA
#SBATCH --output=logs/slurm_paper1_EDM_expA_%x_%j.log
#SBATCH --error=logs/slurm_paper1_EDM_expA_%x_%j.err
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -m sbgm.cli.main_app --mode quicklook --config_path '$CONFIG_DIR/paper1_expA.yaml' --make_plots &&
  python -m sbgm.cli.main_app --mode generate  --config_path '$CONFIG_DIR/paper1_expA.yaml' --make_plots &&
  python -m sbgm.cli.main_app --mode evaluate  --config_path '$CONFIG_DIR/paper1_expA.yaml' --make_plots 
"
